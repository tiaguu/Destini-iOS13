...
struct StoryBrain {
    ...
    mutating func nextStory(_ userChoice : String) {
        if userChoice == stories[currentStorie].choice1 {
            currentStorie = stories[currentStorie].choice1Destination
        } else {
            currentStorie = stories[currentStorie].choice2Destination
        }
    }
    
    func currentStoryTitle() -> String {
        return stories[currentStorie].storyTitle
    }
    
    func updateChoice(_ choiceNumber : Int) -> String {
        if choiceNumber == 1 {
            return stories[currentStorie].choice1
        } else {
            return stories[currentStorie].choice2
        }
